...
// ESPN's API endpoint for Ligue 1 standings
const ESPN_LIGUE1_API = 'https://site.api.espn.com/apis/v2/sports/soccer/fra.1/standings';

async function scrapeLigue1Standings() {
  try {
    console.log('Fetching Ligue 1 standings from ESPN API...');
    
    const response = await axios.get(ESPN_LIGUE1_API, {
      headers: {
    ...
    console.error('Error scraping Ligue 1 standings:', error.message);
    if (error.response) {
...
async function saveToFirestore(standings) {
  try {
    ...
    const collectionRef = db.collection('Ligue1Standings');
...
async function main() {
  try {
    const standings = await scrapeLigue1Standings();
    console.log('\n✅ Ligue 1 standings updated successfully!');
    process.exit(0);
  } catch (error) {
    console.error('\n❌ Failed to update Ligue 1 standings');
    process.exit(1);
  }
}
