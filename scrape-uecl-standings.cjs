...
// ESPN's API endpoint for UEFA Conference League standings
const ESPN_UECL_API = 'https://site.api.espn.com/apis/v2/sports/soccer/uefa.europa.conf/standings';

async function scrapeUECLStandings() {
  try {
    console.log('Fetching UEFA Conference League standings from ESPN API...');
    
    const response = await axios.get(ESPN_UECL_API, {
      headers: {
    ...
    if (data && data.children) {
      for (const group of data.children) {
        const groupName = group.name || 'League Phase';
    ...
    console.error('Error scraping UEFA Conference League standings:', error.message);
    if (error.response) {
...
async function saveToFirestore(standings) {
  try {
    ...
    const collectionRef = db.collection('UEFAConferenceLeagueStandings');
...
async function main() {
  try {
    const standings = await scrapeUECLStandings();
    console.log('\n✅ UEFA Conference League standings updated successfully!');
    process.exit(0);
  } catch (error) {
    console.error('\n❌ Failed to update UEFA Conference League standings');
    process.exit(1);
  }
}
