...
with open('ncaaw-schedule.txt', 'r') as f:
...
while i < len(lines):
    ...
    # Check for date line - handle both formats:
    # "Monday, November 3" or "November 3, 2025"
    date_match1 = re.match(r'^(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday), (January|February|March|April|May|June|July|August|September|October|November|December) (\d+)', line)
    date_match2 = re.match(r'^(January|February|March|April|May|June|July|August|September|October|November|December) (\d+), \d{4}', line)
    
    if date_match1:
        month_name = date_match1.group(2)
        day = int(date_match1.group(3))
    ...
    if time_match or 'TBD' in line.upper():
        ...
        while i < len(lines):
            ...
            if re.match(r'^[A-Z]{2,3}$', candidate):
                i += 1
                continue
            if ',' in candidate and re.search(r', [A-Z]{2}$', candidate):
                i += 1
                continue
...
# Output CSV to the file
with open('ncaaw-schedule.txt', 'w') as f:
    f.write("Date,Time,Away Team,Home Team,Channel\n")
    for game in games:
        f.write(f"{game['date']},{game['time']},{game['away']},{game['home']},{game['channel']}\n")

print(f"Processed {len(games)} games. CSV saved to ncaaw-schedule.txt")
