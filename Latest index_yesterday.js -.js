...
cd /Users/avery/Downloads/THPORTHIndex/yesterday-scores
gcloud run deploy fetch-yesterday-scores --source . --platform managed --region us-central1 --allow-unauthenticated --set-env-vars="FIREBASE_PROJECT_ID=flashlive-daily-scraper,RAPIDAPI_KEY=1c6421f9acmshe820d0c9faf1cf5p165f88jsnc42711af762d,RAPIDAPI_HOST=flashlive-sports.p.rapidapi.com"
...
curl -X POST https://fetch-yesterday-scores-124291936014.us-central1.run.app/fetchYesterdayScores





Terminal folder: Copy of THPORTHINDEX





Latest index_yesterday.js
...
try {
 ...
 for (const sport of sportsToFetch) {
   const url = `https://${RAPIDAPI_HOST}/v1/events/list?sport_slug=${sport.slug}&locale=en_INT&sport_id=${sport.id}&timezone=-4&indent_days=-1`;
...
try {
 ...
 for (let i = 0; i < 7; i++) {
   ...
   for (const league of leaguesToFetch) {
     const url = `https://${RAPIDAPI_HOST}/v1/events/list?sport_slug=${league.slug}&date=${dateToFetch}&locale=en_INT&sport_id=${league.id}&timezone=-4&indent_days=0&status=finished`;
