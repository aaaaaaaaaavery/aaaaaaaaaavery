...
// ESPN API endpoint for Bundesliga standings
const ESPN_BUNDESLIGA_API = 'https://site.api.espn.com/apis/v2/sports/soccer/ger.1/standings';

async function scrapeBundesligaStandings() {
  try {
    console.log('🏆 Scraping Bundesliga standings...');
    
    const response = await axios.get(ESPN_BUNDESLIGA_API);
    const data = response.data;
    
    if (!data.children || data.children.length === 0) {
      console.log('No Bundesliga standings data found');
      return;
    }
    ...
    console.log(`Found ${standings.length} Bundesliga teams`);
    console.log('Sample data:', standings.slice(0, 3));
    
    await saveToFirestore(standings);
    
  } catch (error) {
    console.error('Error scraping Bundesliga standings:', error.message);
    throw error;
  }
}
...
async function saveToFirestore(standings) {
  try {
    console.log('💾 Saving Bundesliga standings to Firestore...');
    
    // Clear existing data
    const collectionRef = db.collection('BundesligaStandings');
    ...
    await batch2.commit();
    console.log(`✅ Saved ${standings.length} Bundesliga teams to Firestore`);
    
    // Clear cache for this league (if running in browser context)
    if (typeof localStorage !== 'undefined') {
      const cacheKey = `standings_cache_Bundesliga`;
      localStorage.removeItem(cacheKey);
      console.log('🗑️ Cleared Bundesliga standings cache');
    }
...
if (require.main === module) {
  scrapeBundesligaStandings()
    .then(() => {
      console.log('Bundesliga standings scraping completed successfully!');
      process.exit(0);
    })
    .catch(error => {
      console.error('Bundesliga standings scraping failed:', error);
      process.exit(1);
    });
}

module.exports = { scrapeBundesligaStandings };
