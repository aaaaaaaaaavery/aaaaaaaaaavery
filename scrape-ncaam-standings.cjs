...
const SHEET_NAME = 'NCAAM'; // Sheet 2
...
// ESPN's API endpoint for Men's College Basketball standings
const ESPN_NCAAM_API = 'https://site.api.espn.com/apis/v2/sports/basketball/mens-college-basketball/standings';

// NCAA API endpoint for Top 25 rankings
const NCAA_RANKINGS_API = 'https://ncaa-api.henrygd.me/rankings/basketball-men/d1/associated-press';

// Team name mapping for better matching (maps variations to canonical names)
...
function normalizeTeamName(name) {
  ...
  // Apply team name mappings (e.g., "Michigan St." -> "Michigan State")
  if (TEAM_NAME_MAPPINGS[normalized]) {
    normalized = TEAM_NAME_MAPPINGS[normalized];
  }
...
async function scrapeNCAAMStandings() {
  try {
    console.log('Fetching NCAAM standings from ESPN API...');
    
    // Fetch Top 25 rankings first
    const rankingsMap = await fetchTop25Rankings();
    
    const response = await axios.get(ESPN_NCAAM_API, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
      }
    });
    ...
    console.error('Error scraping NCAAM standings:', error.message);
    if (error.response) {
...
async function saveTop25ToFirestore() {
  try {
    ...
    // Write to Firestore
    const collectionRef = db.collection('NCAAMStandings');
...
async function saveToFirestore(standings) {
  try {
    ...
    if (typeof localStorage !== 'undefined') {
      const cacheKey = `standings_cache_NCAAM`;
      localStorage.removeItem(cacheKey);
      console.log('🗑️ Cleared NCAAM standings cache');
    }
...
async function main() {
  try {
    ...
    const standings = await scrapeNCAAMStandings();
    await saveToFirestore(standings);
    
    console.log('\n✅ NCAAM standings updated successfully!');
    process.exit(0);
  } catch (error) {
    console.error('\n❌ Failed to update NCAAM standings');
    process.exit(1);
  }
}

// Export for use in other scripts
module.exports = { scrapeNCAAMStandings };
