        ...
        if (!snapshot.empty) {
          snapshot.forEach(doc => {
            ...
            if (data.channel) {
              ...
              console.log(`  📋 ${collectionName}: ${data.home} vs ${data.away} → normalized: ${normalizeTeamName(data.home)} vs ${normalizeTeamName(data.away)}`);
            }
          });
  ...
  try {
    ...
    for (const game of games) {
      ...
      if (channel) {
        ...
        console.log(`   FlashLive teams: ${game['Home Team']} vs ${game['Away Team']}`);
        console.log(`   Normalized: ${normalizeTeamName(game['Home Team'])} vs ${normalizeTeamName(game['Away Team'])}`);
