// index-tomorrow.js - Fetches tomorrow's games and writes to Google Sheets only
...
const TOMORROW_SHEET_NAME = process.env.TOMORROW_SHEET_NAME;
...
const fetchUpcoming7DaysHandler = async (req, res) => {
  try {
    ...
    if (res) res.status(500).send('Upcoming 7 days fetch failed.');
  }
};
...
app.get('/fetchUpcoming7Days', fetchUpcoming7DaysHandler);
app.post('/fetchUpcoming7Days', fetchUpcoming7DaysHandler);

const PORT = process.env.PORT || 8081;
app.listen(PORT, () => {
  console.log(`Upcoming 7 days games server listening on port ${PORT}`);
});
