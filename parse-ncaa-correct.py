...
while i < len(lines):
    ...
    # Skip header
    if 'Time' in line or ('Game' in line and 'TV' in line):
        i += 1
        continue
    ...
    if re.match(r'^\d{1,2}:\d{2}\s*(am|pm)$', line, re.IGNORECASE):
        ...
        while i < len(lines):
            ...
            if ',' in candidate and re.search(r', [A-Z]{2}$', candidate):  # Location
                i += 1
                continue
            away_team = re.sub(r'^\d+\s+', '', candidate)  # Remove leading ranking
            i += 1
            break
        ...
        while i < len(lines):
            ...
            if re.match(r'^[A-Z]{2,3}$', candidate):  # State codes
                i += 1
                continue
            # Check if it's a channel (contains common channel keywords)
            if any(x in candidate.upper() for x in ['ESPN', 'ACC', 'SEC', 'BTN', 'CBS', 'FOX', 'TNT', 'NBC', 'PEACOCK', 'YOUTUBE', 'FLOHOPS', 'MW NETWORK', 'NEC FRONT ROW', 'MNMT', 'MIDCO', 'SPECTRUM', 'BALLERTV', 'TBD', 'TV DATA', 'HBO MAX', 'TRUTV', 'PARAMOUNT', 'SNY', 'MASN', 'CW']):
                break
            if any(kw in candidate for kw in skip_keywords):
                i += 1
                continue
            home_team = re.sub(r'^\d+\s+', '', candidate)  # Remove leading ranking
            i += 1
            break
